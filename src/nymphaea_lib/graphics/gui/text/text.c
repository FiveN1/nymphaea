...
np_gui_text* np_gui_text_create(np_gui_context* gui_context, char* text_string) {
    ...
    */
    ...
    // set text shader
    np_shader_data* data = np_mesh_set_shader(np_text_get_mesh(&gui_text->text), &gui_context->default_text_shader);
    *(np_texture_2d*)np_shader_data_get(data, "glyph") = np_font_get_atlas(&gui_context->default_font);
    *(mat3**)np_shader_data_get(data, "model_matrix") = np_tr2_get(&gui_text->transform);
    *(mat3**)np_shader_data_get(data, "view_matrix") = np_tr2_get(&gui_context->camera.transform);
...
void np_gui_text_project_point(np_camera_2d* camera, np_tr2* transform, float in_x, float in_y, float* out_x, float* out_y) {
    // reverse projection
    ...
    // reverse view matrix
    mat3 m3;
    glm_mat3_inv(*np_tr2_get(np_camera_2d_get_transform(camera)), m3);
    glm_mat3_mulv(m3, point_projected, point_projected);
    // revers model matrix
    glm_mat3_inv(*np_tr2_get(transform), m3);
    glm_mat3_mulv(m3, point_projected, point_projected);
    // return
