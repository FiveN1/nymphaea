...
void np_dsa_texture_2d_create_from_file(np_dsa_texture_2d* dsa_texture_2d, GLenum format, const char* filename) {
    ...
    // create the texture
    np_dsa_texture_2d_create(dsa_texture_2d, np_image_get_width(&image), np_image_get_height(&image), format, image_format, np_image_get_data(&image));
    // delete the temporary image data once it is on the GPU
    np_image_delete(&image);
}

void np_dsa_texture_2d_create(np_dsa_texture_2d* dsa_texture_2d, GLuint width, GLuint height, GLenum format, GLenum pixels_format, void* pixels) {
    // create empty texture
    np_dsa_texture_2d_create_empty(dsa_texture_2d, width, height, format);
    // set image data
    glTextureSubImage2D(*dsa_texture_2d, 0, 0, 0, width, height, pixels_format, GL_UNSIGNED_BYTE, pixels);
}

void np_dsa_texture_2d_create_empty(np_dsa_texture_2d* dsa_texture_2d, GLuint width, GLuint height, GLenum format) {
    ...
    glTexParameteri(*dsa_texture_2d, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(*dsa_texture_2d, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    // Nastaví parametry textury (S a T) = (X a Y)
    glTexParameteri(*dsa_texture_2d, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(*dsa_texture_2d, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    // set texture data
